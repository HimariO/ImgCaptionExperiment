...
class CocoDataSet(object):
    ...
    def __init__(self,
        ...
        # self.annotation_result_path = './data/annotations.pickle'
        self.current_id = 0
    ...
    def __next__(self):
        if self.current_id < self.image_num:
            ...
            result = self.GetFeats(start_img=self.current_id, end_img=end_num)
            self.current_id += self.batch_size
            return result
        else:
            raise StopIteration

    def GetFeats(start_img=0, end_img=2000, save_npy=False):
        """
        ...
        ex: [batch_feat_id for batch_feature_id in CocoDataSet]
        """
        ...
        with tf.Session() as sess:
            ...
            for ind, path in zip(range(start_img, end_img), self.image_paths[start_img: end_img]):
                ...
                try:
                    img = utils.load_image(path)digran
